...
class Bot(commands.Cog, name="봇"):
    ...
    @commands.command(name="ping", aliases=["핑"], help="테스트", usage="`?ping`")
    ...
    @commands.check(Main.isOwner)
    async def evaluate(self, ctx: commands.Context, *, arg):
        await ctx.send(eval(arg))

    @evaluate.error
    async def evaluateError(self, ctx: commands.Context, error):
