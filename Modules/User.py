...
class User(commands.Cog):
    ...
    async def setProfile(self, ctx, *Description):
        if Description is None:
            ...
            with open(f"{Path}/User/Profile/{ctx.author.id}.txt", "w") as File:
                File.write(" ")
    ...
    async def profile(self, ctx:commands.Context, target:discord.User=None):
        ...
        try:
            if target is None:
                with open(f"{Path}/User/Profile/{ctx.author.id}.txt", "r") as File:
                    Embed = discord.Embed(
                ...
                with open(f"{Path}User/Profile/{target.id}", "r") as File:
                    Embed = discord.Embed(
